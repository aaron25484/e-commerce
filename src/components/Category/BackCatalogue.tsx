...
const BackCataloguePage: React.FC = () => {
  ...
  const renderBackCatalogue = () => {
    const backCatalogue = products.filter(
      (product) => product.category === "Back to Catalogue"
    );

    return (
      <div>
        <h2 className="text-2xl font-slackey font-bold text-yellow-500">
          Back in Catalogue
        </h2>
            ...
            backCatalogue.map((product) => (
              <div key={product.id} className="flex-col items-center m-4">
                <Link to={`/${product.id}`}>
                  ...
                  <p className="mt-1 text-center text-yellow-200">
                    {product.artist}
                  </p>
                  <p className="mt-1 text-center text-yellow-600">
                    {product.price} €
                  </p>
                </Link>
              </div>
